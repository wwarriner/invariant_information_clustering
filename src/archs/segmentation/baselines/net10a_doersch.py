...
from src.archs.segmentation.net10a import SegmentationNet10aTrunk, SegmentationNet10a
...
class DoerschHead(nn.Module):
    def __init__(self, config):
        ...
        self.siamese_branch = nn.Sequential(
            nn.Conv2d(
                in_channels=SegmentationNet10a.cfg[-1][0],
                out_channels=1024,
                kernel_size=3,
                stride=1,
                padding=1,
    ...
    def forward(self, patches1, patches2):
        ...
        if not (
            ...
            and (h == h2)
            and (w == w2)
            and (h == self.patch_side)
            and (w == self.patch_side)
        ):
            print((ni, k, h, w))
            print((ni2, k2, h2, w2))
            assert False
        ...
        if not ((ni3 == ni) and (nf == (2 * 1024 * self.patch_side * self.patch_side))):
            print((ni, k, h, w))
            print((ni2, k2, h2, w2))
            print(patches1.size())
            print(patches2.size())
...
class SegmentationNet10aDoersch(VGGNet):
    def __init__(self, config):
        ...
        print(
            (
                "SegmentationNet10aDoersch: %d %d %d"
