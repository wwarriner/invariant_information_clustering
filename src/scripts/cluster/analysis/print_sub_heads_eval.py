...
def main():
    ...
    parser.add_argument(
        "--model_inds", type=int, nargs="+", default=[570, 569, 640, 579, 685]
    )
    parser.add_argument(
    ...
    for model_ind in given_config.model_inds:
        ...
        dataloaders_head_A, dataloaders_head_B, mapping_assignment_dataloader, mapping_test_dataloader = cluster_twohead_create_dataloaders(
            config
        )
        ...
        get_subhead_using_loss(
            config, dataloaders_head_B, net, sobel, lamb, compare=True
        )
