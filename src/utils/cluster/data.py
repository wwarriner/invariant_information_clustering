...
def make_STL_data(
    config, tf1=None, tf2=None, tf3=None, truncate_assign=False, truncate_pc=None
):
    ...
    mapping_assignment_dataloader = _create_mapping_loader(
        ...
        tf3,
        partitions=config.mapping_assignment_partitions,
        truncate=truncate_assign,
        truncate_pc=truncate_pc,
    )
...
def make_MNIST_data(
    ...
    mapping_test_dataloader = _create_mapping_loader(
        config, torchvision.datasets.MNIST, tf3, config.mapping_test_partitions
    )
...
def _create_dataloaders(
    config, dataset_class, tf1, tf2, partitions, target_transform=None, shuffle=False
):
    train_imgs_list = []
    ...
    if not shuffle:
        assert isinstance(
            train_dataloader.sampler, torch.utils.data.sampler.SequentialSampler
        )
    ...
    for d_i in range(config.num_dataloaders):
        print(
            (
                "Creating auxiliary dataloader ind %d out of %d time %s"
        ...
        train_tf_dataloader = torch.utils.data.DataLoader(
            train_imgs_tf,
            batch_size=config.dataloader_batch_sz,
            shuffle=shuffle,
            num_workers=0,
        ...
        if not shuffle:
            assert isinstance(
                train_tf_dataloader.sampler, torch.utils.data.sampler.SequentialSampler
            )
...
def _create_mapping_loader(
    ...
    for partition in partitions:
        ...
        if tencrop:
            imgs_curr = TenCropAndFinish(
                imgs_curr, input_sz=config.input_sz, include_rgb=config.include_rgb
            )
