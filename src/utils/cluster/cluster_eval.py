...
def _clustering_get_data(
    config, net, dataloader, sobel=False, using_IR=False, get_soft=False, verbose=0
):
    """
  ...
  """
    ...
    if get_soft:
        soft_predss_all = [
            torch.zeros(
                (num_batches * config.batch_sz, config.output_k), dtype=torch.float32
            ).cuda()
            for _ in range(config.num_sub_heads)
        ]
    ...
    flat_predss_all = [
        flat_predss_all[i][:num_test] for i in range(config.num_sub_heads)
    ]
    ...
    if not get_soft:
        ...
        soft_predss_all = [
            soft_predss_all[i][:num_test] for i in range(config.num_sub_heads)
        ]

        return flat_predss_all, flat_targets_all, soft_predss_all


def cluster_subheads_eval(
    config,
    net,
  ...
  """
    ...
    if config.mode == "IID":
        ...
        for i in range(config.num_sub_heads):
            reordered_preds = torch.zeros(
                num_samples, dtype=flat_predss_all[0].dtype
            ).cuda()
...
def _get_assignment_data_matches(
    net,
    mapping_assignment_dataloader,
    config,
    sobel=False,
  ...
  """
    ...
    if verbose:
        ...
        print(
            "flat_targets_all %s, flat_predss_all[0] %s"
    ...
    for i in range(config.num_sub_heads):
        if verbose:
            print(
                ...
                % (i, config.eval_mode, datetime.now())
            )
            sys.stdout.flush()
        ...
        if not just_matches:
            ...
            for pred_i, target_i in match:
                reordered_preds[flat_predss_all[i] == pred_i] = torch.as_tensor(
                    target_i.astype(np.uint8)  # ! something is wrong with DTYPE here
                )
                found[pred_i] = 1
                if verbose == 2:
...
def get_subhead_using_loss(config, dataloaders_head_B, net, sobel, lamb, compare=False):
    ...
    for tup in zip(*iterators):
        ...
        all_imgs = torch.zeros(
            config.batch_sz, dim, config.input_sz, config.input_sz
        ).cuda()
        ...
        for d_i in range(config.num_dataloaders):
            ...
            all_imgs_tf[
                actual_batch_start:actual_batch_end, :, :, :
            ] = imgs_tf_curr.cuda()
    ...
    if compare:
        ...
        print(
            "... loss select acc: %f, eval select acc: %f"
...
def cluster_eval(
    ...
    if config.double_eval:
        ...
        stats_dict2 = cluster_subheads_eval(
            config,
            net,
